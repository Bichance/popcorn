...
<body>
	<div class="container">
		<h1 class="col-sm-offset-2 col-sm-10">질문게시판</h1>
		<form class="form-inline" action="./list" method="post">
			<div class="form-group">
				<select class="form-control" name="col">
					<option value="id" <c:if test="${col=='qna_id'}">selected</c:if>>ID</option>
					...
					<option value="qna_title_qna_content"
						<c:if test="${col=='qna_title_qna_content'}">selected</c:if>>제목+내용</option>
		...
		<table class="table table-striped">
			...
			<tbody>
				<c:choose>
					<c:when test="${empty list}"><tr><td colspan="5">등록된 게시판정보가 없습니다.</td></tr></c:when>
					<c:otherwise>
